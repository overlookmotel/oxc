...
impl<'a> Lexer<'a> {
    ...
    pub(super) unsafe fn identifier_name_handler(&mut self) -> &'a str {
        ...
        byte_search! {
            ...
            ret_type: &'a str,
            start: after_first,
            handle_match: |lexer, next_byte, after_first| {
                ...
                if !next_byte.is_ascii() {
                    #[cold]
                    fn unicode<'a>(lexer: &mut Lexer<'a>, after_first: SourcePosition) -> &'a str {
                        ...
                        &lexer.identifier_tail_unicode(start_pos)[1..]
                    }
                    return unicode(lexer, after_first);
                }
                if next_byte == b'\\' {
                    #[cold]
                    fn backslash<'a>(lexer: &mut Lexer<'a>, after_first: SourcePosition) -> &'a str {
                        // SAFETY: `after_first` is position after consuming 1 byte, so subtracting 1
                        // makes `start_pos` `source`'s position as it was at start of this function
                        let start_pos = unsafe { after_first.sub(1) };
                        &lexer.identifier_backslash(start_pos, false)[1..]
                    }
                    return backslash(lexer, after_first);
                }
                ...
                unsafe { lexer.source.str_from_pos_to_current_unchecked(after_first) }
            },
            handle_eof: |lexer, after_first| {
                // Return identifier minus its first char.
    ...
    pub fn identifier_backslash_handler(&mut self) -> &'a str {
        ...
        self.identifier_on_backslash(str, true)
    }
    ...
    pub fn private_identifier(&mut self) -> Kind {
        ...
        if start_pos.addr() == self.source.end_addr() {
            #[cold]
            fn eof(lexer: &mut Lexer) -> Kind {
                let start = lexer.offset();
                lexer.error(diagnostics::UnexpectedEnd(Span::new(start, start)));
                Kind::Undetermined
            }
            return eof(self);
        }
        ...
        byte_search! {
            ...
            handle_match: |lexer, next_byte, after_first| {
                ...
                if !next_byte.is_ascii() {
                    #[cold]
                    fn unicode(lexer: &mut Lexer, after_first: SourcePosition) -> Kind {
                        // SAFETY: `after_first` is position after consuming 1 byte, so subtracting 1
                        // makes `start_pos` `source`'s position as it was at start of this function
                        let start_pos = unsafe { after_first.sub(1) };
                        lexer.identifier_tail_unicode(start_pos);
                ...
                if next_byte == b'\\' {
                    #[cold]
                    fn backslash(lexer: &mut Lexer, after_first: SourcePosition) -> Kind {
                        // SAFETY: `after_first` is position after consuming 1 byte, so subtracting 1
                        // makes `start_pos` `source`'s position as it was at start of this function
                        let start_pos = unsafe { after_first.sub(1) };
                        lexer.identifier_backslash(start_pos, false);
                        Kind::PrivateIdentifier
                    }
                    return backslash(lexer, after_first);
                }
