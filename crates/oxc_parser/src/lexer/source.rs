...
///
/// # SAFETY
/// `SourcePosition` must always be on a UTF-8 character boundary, and within bounds of the `Source`
/// that created it.
...
impl<'a> SourcePosition<'a> {
    ...
    /// Get string slice from between this `SourcePosition` and another `SourcePosition`.
    ///
    /// # SAFETY
    /// `end` must not be before `self`.
    // TODO: Delete this if not using it
    #[allow(dead_code)]
    #[inline]
    pub(super) unsafe fn str_up_until(self, end: Self) -> &'a str {
        // SAFETY: Caller guarantees `end` is not before `self`.
        // `SourcePosition`s can only be created from a `Source`.
        // `Source::new` takes a `UniquePromise`, which guarantees that it's the only `Source`
        // in existence on this thread. `Source` is not `Sync` or `Send`, so no possibility another
        // `Source` originated on another thread can "jump" onto this one.
        // This is sufficient to guarantee that any two `SourcePosition`s that parser/lexer hold must be
        // from a single `Source`.
        // Therefore `self.ptr` and `end.ptr` must both be within the same allocation and derived
        // from the same original pointer.
        // Invariant of `SourcePosition` type guarantees that both are positioned on UTF-8 character
        // boundaries. So slicing source text between these 2 points will always yield a valid
        // UTF-8 string.
        debug_assert!(self.ptr <= end.ptr);
        let len = end.addr() - self.addr();
        let slice = slice::from_raw_parts(self.ptr, len);
        std::str::from_utf8_unchecked(slice)
    }
}
