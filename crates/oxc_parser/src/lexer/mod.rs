...
impl<'a> Lexer<'a> {
    ...
    fn read_next_token(&mut self) -> Kind {
        ...
        // Slice off a leading space if there is one, without a branch.
        // Equivalent to `self.current.chars = remaining.strip_prefix(' ').unwrap_or(remaining).chars()`
        // but a few less instructions.
