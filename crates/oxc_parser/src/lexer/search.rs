...
///         ret_type: (),
///         handle_match: |lexer, matched_byte, start| {},
///         handle_eof: |lexer, start| {},
///       };
///     };
///   }
/// }
...
///         //
///         // `lexer` is mutable reference to `Lexer`.
...
macro_rules! byte_search {
    ...
    (
        ...
        handle_match: |$match_lexer:ident, $match_byte:ident, $match_start:ident| $match_handler:expr,
        handle_eof: |$eof_lexer:ident, $eof_start:ident| $eof_handler:expr,
    ) => {{
        ...
        byte_search! {
            ...
            ret_type: $ret_type,
            start: start,
            handle_match: |$match_lexer, $match_byte, $match_start| $match_handler,
            handle_eof: |$eof_lexer, $eof_start| $eof_handler,
        }
    }};
    ...
    (
        ...
        loop {
            if pos.addr() > lexer.source.end_for_batch_search_addr() {
                ...
                return unbatched(lexer, start, pos);
            }
            ...
            // No match in batch - loop round and continue searching next batch
        }

        /// Search tail end of source, byte by byte
