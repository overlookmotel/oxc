...
impl<'a> Lexer<'a> {
    ...
    pub(super) fn read_non_decimal(&mut self, kind: Kind) -> Kind {
        self.current.chars.next();
    ...
    pub(super) fn read_legacy_octal(&mut self) -> Kind {
    ...
    pub(super) fn read_decimal_exponent(&mut self) -> Kind {
    ...
    pub(super) fn read_decimal_digits(&mut self) {
    ...
    pub(super) fn read_decimal_digits_after_first_digit(&mut self) {
        while let Some(c) = self.peek() {
            match c {
                '_' => {
                    self.current.chars.next();
    ...
    pub(super) fn decimal_literal_after_decimal_point_after_digits(&mut self) -> Kind {
        self.optional_decimal_digits();
        self.optional_exponent();
        self.check_after_numeric_literal(Kind::Float)
    }

    pub(super) fn optional_decimal_digits(&mut self) {
        if self.peek().is_some_and(|c| c.is_ascii_digit()) {
            self.current.chars.next();
        } else {
            return;
    ...
    pub(super) fn optional_exponent(&mut self) -> Option<Kind> {
    ...
    pub(super) fn check_after_numeric_literal(&mut self, kind: Kind) -> Kind {
