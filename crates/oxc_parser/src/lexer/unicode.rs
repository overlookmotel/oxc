...
impl<'a> Lexer<'a> {
    ...
    ///   \u{ `CodePoint` }
    pub(super) fn string_unicode_escape_sequence(
        &mut self,
        text: &mut String<'a>,
    ...
    pub(super) fn unicode_code_point(&mut self) -> Option<SurrogatePair> {
        if !self.next_eq('{') {
            return None;
        }
    ...
    pub(super) fn hex_4_digits(&mut self) -> Option<u32> {
        let mut value = 0;
        for _ in 0..4 {
    ...
    pub(super) fn hex_digit(&mut self) -> Option<u32> {
        let value = match self.peek() {
    ...
    pub(super) fn code_point(&mut self) -> Option<u32> {
    ...
    pub(super) fn surrogate_pair(&mut self) -> Option<SurrogatePair> {
